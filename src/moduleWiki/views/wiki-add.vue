...
import showMachine from "../stateMachins/displayWikiAdd.js";
...
const wikiAdd = {
  ...
  methods: {
    noContent() {
      this.$store.commit(
        ...
        {
          header: "Ошибка",
          text: "Заполните все данные для публикации статьи"
        },
        { root: true }
      );
      this.$store.commit("SHOW_ALERT", { root: true });
      this.showMachine.editing();
    },
    async hasContent() {
      this.newPost.content = this.newPostContent.root.innerHTML;
      this.newPost.url = this.$route.params.name;
    ...
    async hasArticle() {
      this.showMachine.opening();
      await this.$store.dispatch("GET_WIKI", this.$route.params.name);
  ...
  async created() {
    ...
    this.$store.dispatch("GET_TAGS", this.$route.params.name);
